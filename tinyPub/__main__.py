...
class impPYCUI(pycui.PyCUI):
    ...
    def add_Displayer(self, title, row, column, row_span = 1, column_span = 1, padx = 1, pady = 0, book = None):
        ...
        new_displayer = Displayer(id, title,  self._grid, row, column, row_span, column_span, padx, pady, book)
        self._widgets[id] = new_displayer
    ...
    def add_Adnotation_menu(self, title, row, column, row_span = 1, column_span = 1, padx = 1, pady = 0):
        id = 'Widget{}'.format(len(self._widgets.keys()))
        new_adnotation_menu = Adnotation_menu(id, title, self._grid, row, column, row_span, column_span, padx, pady, self._logger)
        self._widgets[id] = new_adnotation_menu
        if self._selected_widget is None:
            self.set_selected_widget(id)
...
class Displayer(pycui.widgets.ScrollTextBlock):
    ...
    def __init__(self, id, title, grid, row, column, row_span, column_span, padx, pady, book):
        ...
        last = None
        while self._cursor_text_pos_y < self.book.XcursorPos:
            last = self._cursor_text_pos_y
            self.move_down()
            if last == self._cursor_text_pos_y:
                # if cursor position is  not changing, well, you tried
                break
        while self._cursor_text_pos_x < self.book.YcursorPos:
            last = self._cursor_text_pos_x
            self._move_right()
            if last == self._cursor_text_pos_x:
                break
    ...
    def add_adnotation_on_cursor(self, text):
        ...
        adnotation = self.book.add_adnotation(text, self._cursor_text_pos_y, self.breaks)
    ...
    def append_progress_to_title(self):
        self.title = self.org_title + ' '+ str(max(round(100*self._cursor_text_pos_y / len(self._text_lines)), 0)) + '%'

    def jump_to_line(self, line):
        """change cursor_text_pos_y to line"""
        if self._cursor_text_pos_y > line:
            last = self._cursor_text_pos_y
            while line < self._cursor_text_pos_y:
                last = self._cursor_text_pos_y
                self.move_up()
                if last == self._cursor_text_pos_y:
                    # if position has not changed, abort
                    break
        elif self._cursor_text_pos_y < line:
            last = self._cursor_text_pos_y
            while self._cursor_text_pos_y < line:
                last = self._cursor_text_pos_y
                self.move_down()
                if last == self._cursor_text_pos_y:
                    # if position has not changed, abort
...
class Interface:
    ...
    def __init__(self, master, book):
        ...
        self.master._grid.offset_x = 0
        self.master._grid.offset_y = 0
        # calculate width for given lineLength in config for Displayer()
        lineLength = config.get('lineLength', 78)
        min_columns = (lineLength + 15) // self.master._grid._column_width + 1
        try:
            self.textDisplayer = self.master.add_Displayer('' ,row = 0, column = 0, row_span = 5, column_span = min_columns, book = self.book)
        except:
            self.textDisplayer = self.master.add_Displayer('' ,row = 0, column = 0, row_span = 5, column_span = self.master._grid._num_columns, book = self.book)
        if min_columns + 2 < self.master._grid._num_columns:
            self.addnotationsAllowed = True
            self.addnotationsMenu = self.master.add_Adnotation_menu('Notes', row = 0, column = min_columns, row_span = 5, column_span = self.master._grid._num_columns - min_columns)
